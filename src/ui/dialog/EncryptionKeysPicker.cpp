...
#include "EncryptionKeysPicker.h"
...
EncryptionKeysPicker::EncryptionKeysPicker(int channel, QWidget* parent)
    : EncryptionKeysPicker(channel, {}, parent) {}

EncryptionKeysPicker::EncryptionKeysPicker(
    int channel, const GpgAbstractKeyPtrList& restrict_keys, QWidget* parent)
    : GeneralDialog("EncryptionKeysPicker", parent),
      channel_(channel),
      tree_view_(new KeyTreeView(
          ...
          [allowed = CollectFingerprints(restrict_keys)](
            ...
            // enough. Revoked subkeys are never offered.
            if (k->KeyType() == GpgAbstractKeyType::kGPG_SUBKEY) {
              return k->IsHasEncrCap() && !k->IsRevoked();
            }
            if (!k->IsHasEncrCap()) return false;
  ...
  setWindowTitle(tr("Select Encryption Key(s)"));
  setModal(true);
  setMinimumSize(560, 420);
  resize(700, 500);

  auto* title_label = new QLabel(tr("Choose Encryption Key(s) or Subkey(s)"));
  auto title_font = title_label->font();
  title_font.setBold(true);
  title_label->setFont(title_font);

  auto* desc_label = new QLabel(
      tr("Select the recipient key(s) or specific encryption subkey(s) to use "
         "for this operation. Expand a key to choose a particular subkey."));
  ...
  connect(button_box, &QDialogButtonBox::accepted, this, [this]() {
    if (tree_view_ == nullptr || tree_view_->GetAllCheckedKeys().isEmpty()) {
      QMessageBox::information(
          ...
          tr("Please select at least one encryption key or subkey."));
      return;
    }
    accept();
  });
  ...
  connect(tree_view_, &KeyTreeView::SignalKeysChecked, this,
          &EncryptionKeysPicker::update_confirm_button_state);
...
auto EncryptionKeysPicker::GetEncryptionKeys() const -> GpgAbstractKeyPtrList {
